package priv.jssse.mall.filter;
...
public class AuthorizationFilter implements Filter {
    ...
    public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain) throws IOException, ServletException {
        ...
        if (path.endsWith(".do") || path.endsWith(".html")) {
            ...
            if (path.endsWith("toLogin.html")
                    ...
                    || path.contains("/priv/jssse/mall/admin/product/img/")
                    || path.endsWith("index.html")
                    || path.endsWith("classification/list.do")
                    || path.contains("product")
                    || path.contains("/priv/jssse/mall/h2-console")) {
    ...
    private void processAccessControl(HttpServletRequest request, HttpServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        if (url.indexOf("admin") != -1){
            if (adminUser == null) {
                response.sendRedirect("/priv/jssse/mall/admin/toLogin.html");
            }else {
                chain.doFilter(request, response);
            }
        }else {
            if (user == null) {
                response.sendRedirect("/priv/jssse/mall/user/toLogin.html");
            }else {
                chain.doFilter(request, response);
            }
        }
